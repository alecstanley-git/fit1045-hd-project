...
class Figure
{
    ...
    Camera *camera = nullptr; // Pointer to the camera object initialised on startup
    ...
    // Used in 2D mode, this determines how far in from the plot walls we want to draw our data
    double padding_multiplier = 0.1; // 0.1 corresponds to 10% margins

    // The point struct allows us to associate a color and radius to each point in space
    struct Point
    {
    ...
    // Maps a clip-space point (post viewProj, pre perspective-divide) to screen pixels.
    // Assumes clip.w > 0.
    ...
    // World Vec3 to screen pixels. Returns false if the point is behind the near plane.
    ...
    // projects a line segment, clipping it against the near plane in clip space first.
    // returns false if the whole segment is behind the camera.
    // @param const Mat4 &viewProj - combined view-projection matrix
    // @param const Vec3 &w1 - first endpoint in world space
    // @param const Vec3 &w2 - second endpoint in world space
    // @param Point2D &s1 - output parameter that receives the first screen endpoint
    // @param Point2D &s2 - output parameter that receives the second screen endpoint
    // @return bool - false if the whole segment is behind the camera, true otherwise
    bool project_segment(const Mat4 &viewProj, const Vec3 &w1, const Vec3 &w2, Point2D &s1, Point2D &s2);

    // Liang-Barsky algorithm: trims segment (x1,y1)-(x2,y2) to the figure rectangle in place.
    // Returns false if the segment lies entirely outside the box.
    // @param int &x1, &y1 - first endpoint, trimmed in place to the figure rectangle
    // @param int &x2, &y2 - second endpoint, trimmed in place to the figure rectangle
    // @return bool - false if the segment lies entirely outside the box, true otherwise
    ...
    static TickSpec compute_ticks(const double &min, const double &max, const int target = TICK_COUNT);
...
public:
    ...
    void set_xlim(const double min, const double max) { xlim = {min, max}; }   // Set the x-axis range
    void set_ylim(const double min, const double max) { ylim = {min, max}; }   // Set the y-axis range
    void set_zlim(const double min, const double max) { zlim = {min, max}; }   // Set the z-axis range (3D only)
    void set_padding(const double _padding) { padding_multiplier = _padding; } // Set the margin between data and plot walls
    void set_title(const std::string &_title) { title = _title; }              // Set the figure title
    void set_xlabel(const std::string &_label) { xlabel = _label; }            // Set the x-axis label
    void set_ylabel(const std::string &_label) { ylabel = _label; }            // Set the y-axis label
    void set_zlabel(const std::string &_label) { zlabel = _label; }            // Set the z-axis label (3D only)
