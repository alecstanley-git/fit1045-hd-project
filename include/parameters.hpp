...
{
    // Simulation params
    inline constexpr double TIME_STEP = 0.005;              // Simulation time between steps (unitless)
    ...
    inline Scale SCALE = SOLAR_SYSTEM;                      // initial scale - SOLAR_SYSTEM or INTERGALACTIC
    inline const std::string CONFIG_DIR = "configurations"; // directory scanned for .json configs at runtime
    ...
    inline constexpr float CAM_FOV = 50.0f * Constants::pi / 180.0f; // Field of view in radians
    inline constexpr float CAM_ASPECT = 1.3333f;                     // Keep to 4/3 unless absolutely necessary
    inline constexpr float CAM_ZNEAR = 0.1f;                         // Objects must be at least this far in front of camera to render
    inline constexpr float CAM_ZFAR = 50.0f;                         // Objects further than this number in front of camera won't render
    inline constexpr float SCROLL_ZOOM_FACTOR = 0.1f;                // Increase for faster scrolling on 3D plot
    ...
    inline constexpr double AXIS_3D_LOWER = -5;       // currently no way to adjust 3D axis limits during runtime, but objects will render beyond axes in 3D (unlike 2D)
    inline constexpr double AXIS_3D_UPPER = 5;
    inline double AXIS_2D_LOWER = -1.5; // initial 2D axis bounds, can be zoomed in/out during runtime
