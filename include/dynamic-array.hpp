...
/*
dynamic_array is a custom-made version of std::vector. It is a resizable array that dynamically grows and shrinks.
*/
...
class dynamic_array
{
    ...
    T *data;       // The pointer to the memory chunk containing all the data in the array

    // The reallocate method is the private method that handles expansion/reduction of memory
    // @param int new_capacity - the target new total capacity the array should have, can be higher or lower
    void reallocate(int new_capacity)
    {
        ...
        for (int i = 0; i < _size; i++)
        {
            // The normal new command allocates memory and constructs the object automatically
            // But since we already did that with malloc, we have to use the 'placement new'
            // This syntax is basically like new (address);, meaning don't allocate anything, just
            // construct T at the target address.
            // the std::move part copies the data over, and deletes the old data
...
public:
    ...
    dynamic_array(const dynamic_array &other)
    {
        ...
        if (_capacity > 0)
        {
            ...
            for (int i = 0; i < _size; i++)
            {
                ...
                // We don't delete the old data, because this is explicitly for copying (we are supposed to duplicate elements)
            }
        }
        else
        {
    ...
    // Very similar to copy constructor, but runs when the object being overwritten already exists
    // @param const dynamic_array &other - the array to be copied
    ...
    // The add() method needs to add the data to the array and also increase the capacity if its ful
    // @param T value - the value to add
    void add(T value)
    {
        // If the new element will cause the array to exceed capacity, we double capacity
        if (_size >= _capacity)
        {
            int new_cap = (_capacity == 0) ? 1 : _capacity * 2;
            reallocate(new_cap); // Reallocate is called to map all the data to a new array with a new size
        }

        // Construct the new element directly in the raw memory slot using placement new again
        new (&data[_size]) T(std::move(value));
        _size++;
    }

    // Remvoe does pretty much the opposite from above, but instead of adding a specific value, we assume the user already knows the index of the element they want to remove
    ...
    // Key point: This method is called when you want to reassign an element of the array
    T &get(int index)
    {
        // Check bounds
        if (index < 0 || index >= _size)
    ...
    // Key point: different to above, this is called when you only want to fetch (not change) the array value
    ...
    /*
    In many cases I am using the dynamic array to hold numerical values. In these cases
    it benefits me to allow methods like min and max, but they should only be enabled if the data type is numerical.
    The standard library allows for this by filtering the template type.
    */
    ...
    double min() const
    {
        ...
        // Iterate through the entire array, setting any new lower values to the global min
    ...
    double max() const
    {
        ...
        // Iterate through the entire array, setting any new higher values to the global max
