...
enum Integrator { LEAPFROG };

// Tracks whether the simulation is currently running or paused/stopped
enum SimState { INACTIVE, ACTIVE };

class Simulator {
  // Advances every body one TIME_STEP forward using the leapfrog
  // (kick-drift-kick) scheme
  void leapfrog();
  // Advances every body one TIME_STEP backward, exploiting leapfrog's
  // time-reversibility
...
public:
  ...
  int current_step =
      0; // Index of the current integration step (0 at simulation start)
  ...
  // @param const bool &override_time - if true, ignores the SIM_TIME upper
  // bound
...
inline void Simulator::fill_console() {
  // Prompt user in console for all the values
  std::cout << std::endl;
  int to_add =
      read_integer_range("How many masses in the simulation? ", 1, INT_MAX);
  std::cout << std::endl;
  ...
  std::cout << std::endl
            << "[-] " << n_bodies << " bodies initialised, ready to commence."
            << std::endl;
}
...
// Lists CONFIG_DIR/*.json, prints a numbered menu to the terminal, and returns
// the chosen file's full path. Returns "" if the directory is missing/empty. I
// made it static so it needs no instance state and runs before the new
// Simulator exists.
inline std::string Simulator::choose_config_file() {
  ...
  if (e || files.length() == 0) {
    std::cout << "No configuration files found in '" << CONFIG_DIR << "'."
              << std::endl;
  ...
  int choice =
      read_integer_range("Select a configuration: ", 1, files.length());
  return files[choice - 1];
}

inline BodyState Simulator::fetch_user_config_console() {
  BodyState config;
  config.mass =
      read_double_range("   Enter mass (solar masses): ", 0, INFINITY, false);
  ...
  bool custom_position = read_boolean(
      "   Want to enter a custom initial position (defaults to origin)?");
  if (!custom_position) {
  ...
  bool custom_velocity = read_boolean(
      "   Want to enter a custom initial velocity (defaults to [1, 0, 0])?");
  if (!custom_velocity) {
...
inline void Simulator::build_rings(const BodyState &params) {
  ...
  for (int i = 1; i < params.rings + 1; i++) {
    ...
    double vphi =
        std::sqrt((params.mass * r_sq) / (dist_sq * std::sqrt(dist_sq)));
...
inline void Simulator::calculate_acceleration() {
  ...
  for (int i = 0; i < n_bodies; i++) {
    ...
    for (int j = 0; j < n_bodies; j++) {
      ...
      // Softened distance
      distance_sq =
          dx.x * dx.x + dx.y * dx.y + dx.z * dx.z + SOFTENING * SOFTENING;
...
inline void Simulator::leapfrog() {
  for (int i = 0; i < n_bodies; i++) {
    bodies[i].data.velocity = bodies[i].data.velocity +
                              bodies[i].data.acceleration * (0.5 * TIME_STEP);
    bodies[i].data.position =
        bodies[i].data.position + bodies[i].data.velocity * TIME_STEP;
  }
...
inline void Simulator::step(Integrator integrator,
                            const bool &override_time = false) {
...
inline void Simulator::step_backward(Integrator integrator,
                                     const bool &override_time = false) {
