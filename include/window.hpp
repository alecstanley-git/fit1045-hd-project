...
class Window {
  ...
  bool is_open =
      false;     // True while the window is alive; flipped false when it closes
  void *_window; // This points to the os-specific window object - must be a
                 // pointer*.

  // Interaction fields
  Point2D mouse_position = {
      0, 0}; // Current mouse position in window pixels, updated each frame
  Point2D previous_mouse_pos = {
      0, 0}; // Mouse position from the previous frame, used to compute velocity
  bool is_mouse_down =
      false; // True while the left mouse button is currently pressed
  bool was_mouse_down = false; // Tracks the previous press state to distinguish
                               // a fresh click from a hold

  // Registers the os-specific event handlers (mouse, scroll) needed for
  // interaction. Only ran once during window construction.
  void setup_input_listeners();

public:
  // The buttons should be a publicly accessible field - they are just pointers
  // to the structs
  dynamic_array<Button *> buttons;
  float zoom_level = 100.0f; // Running zoom total driven by the scroll wheel,
  ...
  */
  ...
  // Pumps the os event queue (clicks, drags, close) and refreshes mouse state.
  // Called every frame.
  void process_events();
  ...
  // @param bool is_button - when true, renders the button styling (defaults to
  // false)
  ...
  // @param int box_width, box_height - dimensions of the box the text is
  // centred within
  void draw_text(const std::string &text, int x, int y, double size,
                 Color color, int box_width, int box_height);
  ...
  // Allocates a new button, stores it in the buttons array, and returns a
  // pointer to it.
  ...
  // Updates the interaction state of the listed buttons and redraws them with
  // the matching style.
  // @param dynamic_array<int> &indices - indices into the buttons array to
  // process this frame
  void process_buttons(dynamic_array<int> &indices);

  // Thin per-frame helper: processes events then sleeps to hit the target frame
  // rate.
  // @param double fps - the target frames per second
  void update_window(double fps);

  // @return Point2D - mouse movement (dx, dy) since the last frame, or {0,0}
...
inline bool Window::is_running() { return is_open; }

inline Button *Window::add_button(int x, int y, int w, int h,
                                  std::string text) {
  ...
  for (int i = 0; i < indices.length(); i++) {
    ...
    if (buttons[idx]->is_hovering(mouse_position)) {
      if (is_mouse_down) {
        ...
        // First frame of the press: register a one-shot CLICKED, then latch
        // was_mouse_down so subsequent frames are treated as HELD rather than
        // repeated clicks
    ...
    fill_rectangle(buttons[idx]->x, buttons[idx]->y, buttons[idx]->width,
                   buttons[idx]->height, box_color, true);
    draw_text(buttons[idx]->text, buttons[idx]->x, buttons[idx]->y,
              BUTTON_TEXTSIZE, text_color, buttons[idx]->width,
              buttons[idx]->height);
  }
}

/*
This is a thin helper procedure that calls the process_events() method and
sleeps the frame for a set time to achieve a target FPS
*/
...
inline Point2D Window::mouse_velocity() {
  ...
  // Only report movement while dragging, so a release doesn't register a
  // phantom jump
  if (!is_mouse_down)
    return {0, 0};
  return {dx, dy};
