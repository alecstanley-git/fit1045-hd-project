...
void Figure::plot(const dynamic_array<double> &x, const dynamic_array<double> &y, const Color &_color, const int _markersize)
{
    ...
    for (int i = 0; i < x.length(); i++)
    {
        ...
        pt.pos.x = static_cast<int>(std::round(x_padding + (x[i] - xlim.x) / x_range * drawable_width));
        pt.pos.y = static_cast<int>(std::round(dimensions.y - y_padding - (y[i] - ylim.y) / y_range * drawable_height));
        pt.pos.z = 0.0;
